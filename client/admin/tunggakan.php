                    ...
                    <?php foreach ($data_tunggakan as $key => $tunggakan) : ?>
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                    ...
                                    <ul>
                                            ...
                                            $sql = "SELECT s.nama FROM tunggakan tk, siswa s WHERE tk.id_siswa = s.id_siswa AND s.id_siswa = '$id_siswa'";
                                            $data_tunggakan_siswa = $db->query($sql) or die($db->error);
                                            $data_tunggakan_siswa->fetch_assoc();
                                            ?>
                                            <?php foreach ($data_tunggakan_siswa as $key => $tunggakan) : ?>
                                                <p><?= $tunggakan['nama'] ?></p>
                                            <?php endforeach ?>
                                        </ul>
                                    </td>
